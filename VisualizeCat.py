...
names=['URL','Category']
df=pd.read_csv(r'C:\Users\Shabab\Desktop\Untitled Folder\URL Classification2.csv',names=names, na_filter=False)
df1 = df[1:200]
df2 = df[201:300]
df3 = df[300:400]
df4 =df[400:450]
...
#read url file
cf=pd.read_csv('URL Classification2.csv')
urls = cf.urls.tolist()

#categorize url's into a list
#iterations=20
#for i in range(iterations):
    #print([urls[i]],gs_clf.predict([urls[i]]))

import csv

csvData = [['URL', 'Category']]
with open('person.csv', 'w', newline='') as csvFile:
    writer = csv.writer(csvFile)
    writer.writerows(csvData)
    iterations=20
    for i in range(iterations):
        row = urls[i], gs_clf.predict([urls[i]])
        writer = csv.writer(csvFile)
        writer.writerow(row)
...
names=['URL','Category']
dx=pd.read_csv('person.csv',names=names, na_filter=False)
dx.Category.value_counts().plot(figsize=(12,5),kind='bar',color='green')
plt.xlabel('Category')
